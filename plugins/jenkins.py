...
def jenkins(text):
    ...
    try:
        request = requests.get("https://jenny.blny.me/job/{}/lastBuild/buildNumber".format(text))
    ...
    latest = request.text
    url = text + "/" + latest
    return "The latest build for {} is #{}, and can be downloaded from https://jenny.blny.me/job/{}".format(text, latest, url)
